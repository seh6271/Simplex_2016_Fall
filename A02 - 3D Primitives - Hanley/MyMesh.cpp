...
void MyMesh::GenerateCone(float a_fRadius, float a_fHeight, int a_nSubdivisions, vector3 a_v3Color)
{
	...
	//center points of bases
	vector3 topCenterPoint = vector3(0.0f, a_fHeight / 2, 0.0f);
	vector3 bottomCenterPoint = vector3(0.0f, -(a_fHeight / 2), 0.0f);
	...
	float totalIntAngles = (a_nSubdivisions - 2) * 180;
	//indiviudal interior angles
	float intAngles = totalIntAngles / a_nSubdivisions;
	//center angle (theta)
	...
	//create tris for bases
	for (int i = 0; i < a_nSubdivisions; i++)
	{		
		AddTri(bottomCenterPoint, bottomPoints[i], bottomPoints[i + 1]);
	}

	//create sides
	for (int i = 0; i < a_nSubdivisions; i++)
	{
		AddTri(bottomPoints[i + 1], bottomPoints[i], topCenterPoint);
	}
...
void MyMesh::GenerateCylinder(float a_fRadius, float a_fHeight, int a_nSubdivisions, vector3 a_v3Color)
{
	...
	for (int i = 0; i < a_nSubdivisions; i++)
	{
		AddTri(topCenterPoint, topPoints[i+1], topPoints[i]);
		AddTri(bottomCenterPoint, bottomPoints[i], bottomPoints[i + 1]);
	}
...
void MyMesh::GenerateTube(float a_fOuterRadius, float a_fInnerRadius, float a_fHeight, int a_nSubdivisions, vector3 a_v3Color)
{
	...
	for (int i = 0; i < a_nSubdivisions + 1; i++)
	{
		...
		//add to storage
		topPointsOut.push_back(newTopOuter);
		bottomPointsOut.push_back(newBottomOuter);

		topPointsIn.push_back(newTopInner);
		bottomPointsIn.push_back(newBottomInner);

		//increment angle
		newAngle += theta;
	}

	//create quads for bases
	for (int i = 0; i < a_nSubdivisions; i++)
	{		
		AddQuad(bottomPointsOut[i], bottomPointsOut[i + 1], bottomPointsIn[i], bottomPointsIn[i + 1]);
		AddQuad(topPointsIn[i], topPointsIn[i + 1], topPointsOut[i], topPointsOut[i + 1]);
	}

	//create outsides
	for (int i = 0; i < a_nSubdivisions; i++)
	{
		AddQuad(bottomPointsOut[i + 1], bottomPointsOut[i], topPointsOut[i + 1], topPointsOut[i]);
	}

	//create insides
	for (int i = 0; i < a_nSubdivisions; i++)
	{
		AddQuad(topPointsIn[i + 1], topPointsIn[i], bottomPointsIn[i + 1], bottomPointsIn[i]);
	}
